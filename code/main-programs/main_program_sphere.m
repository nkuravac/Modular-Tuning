...
%freq = 154;  %154 +/- 30   %Table 3 in Darnton et al.
freq_array=[80,154,250];
...
wlmin=0.3*r;
wlmax=12*r; %compare to wl in 0.2 to 9.0
n_wl=24;
hradmin=0.1*r;
hradmax=0.9*r; %compare to R=0.2 or about 0.5*r
n_hrad=21;        

wave_length_array = linspace(wlmin,wlmax,n_wl);
helix_rad_array = linspace(hradmin,hradmax,n_hrad);


for i_freq=1:length(freq_array)
freq=freq_array(i_freq);

purcell_ineff = zeros(length(wave_length_array),length(helix_rad_array));
energy_per_dist = purcell_ineff;
U_model_matrix = purcell_ineff;
flag_torque_matrix = purcell_ineff;
flag_force_matrix = purcell_ineff;

dir = ['../../data/cell_rad',num2str(r),'_freq_',num2str(freq),'_arclen_',num2str(arclen)];
        if(~exist(dir,'dir'))
            mkdir(dir)
        else
            rmdir(dir,'s')
        ...
        freq,arclen,blob_size_on_flag,n_body,filament_radius,mu,wlmin,
        wlmax,n_wl,hradmin,hradmax,n_hrad
        diary off

i_un=0;
for i_wl = 1:length(wave_length_array)
for i_hr = 1:length(helix_rad_array)
%for i_wl=1:splits
    wave_length = wave_length_array(i_wl);
    R = helix_rad_array(i_hr);
    i_un=i_un+1;

    %wave_length_array=[wlmin,(wlmin+wlmax)/2,wlmax];
    %for i_3=1:length(wave_length_array)
    %wave_length=wave_length_array(i_3);
    %for i_cb = 1:length(cell_a)
        %height = height_a;
        
        %{
        dir = ['./wlength_',num2str(wave_length),'_helixrad_',num2str(R),'_arclen_',num2str(arclen)];
        if(~exist(dir,'dir'))
            mkdir(dir)
        else
            rmdir(dir,'s')
        ...
        diary([dir,'/parameters.txt'])
        r,ds_on_cell_body,opti_blob_size_on_cell_body
        Ls,R,freq,arclen,blob_size_on_flag
        diary off
        %}
...
%}       
        for phase = 1:num_phase
            ...
            if phase == 1
                ...
                saveas(fig1,[dir,'/cell_rad_',num2str(r),'_wavelength_',num2str(wave_length_array(i_wl)),'_helix_rad_',num2str(helix_rad_array(i_hr)),'_full_cell.png'])
            end
...
%}
    ...
    %end
%{
    purcell_eff(i_3)=avg_sumFf(3)*avg_U_model(3)/(avg_sumTf(3)*avg_Omega_model(3));
    if purcell_eff(1)<purcell_eff(2) && purcell_eff(3)<purcell_eff(2)
        wlmin=wlmin+(wlmax-wlmin)/4;
        wlmax=wlmax-(wlmax-wlmin)/4;
    elseif purcell_eff(1)<purcell_eff(3)
        wlmin=(wlmin+wlmax)/2;
    else
        wlmax=(wlmin+wlmax)/2;
    end    
...
%}
purcell_ineff(i_wl,i_hr)=(avg_sumTf(3)*(freq*2*pi))/(avg_sumFf(3)*avg_U_model(3));
energy_per_dist(i_wl,i_hr)=(avg_sumTf(3)*(freq*2*pi))/avg_U_model(3);
flag_torque_matrix(i_wl,i_hr)=avg_sumTf(3);
flag_force_matrix(i_wl,i_hr)=avg_sumFf(3);
U_model_matrix(i_wl,i_hr)=avg_U_model(3);

end
end

writematrix(flag_torque_matrix,[dir,'/flag_torque_matrix.txt'])
writematrix(flag_force_matrix,[dir,'/flag_force_matrix.txt'])
writematrix(U_model_matrix,[dir,'/U_model_matrix.txt'])

%% 

min_ineff = min(abs(purcell_ineff(:)));
[min_ineff_wl_index,min_ineff_hrad_index]=find(abs(purcell_ineff)==min_ineff);
opt_wl_purcell = wave_length_array(min_ineff_wl_index)/r;
opt_hrad_purcell = helix_rad_array(min_ineff_hrad_index)/r;

opt_parameters_purcell=fopen([dir,'/scaled_optimal_wavelength_and_helixrad_purcell_ineff_motorspeed_',num2str(2*pi*freq),'.txt'],"w");
fprintf(opt_parameters_purcell,'The minimum Purcell inefficiency is %12.4g\n. The corresponding optimal wavelength (over r) is %12.4g\n The corresponding optimal helix radius (over r) is %12.4g\n The corresponding torque (z-component) is %12.4g\n',min_ineff,wave_length_array(min_ineff_wl_index)/r,helix_rad_array(min_ineff_hrad_index)/r,flag_torque_matrix(min_ineff_wl_index,min_ineff_hrad_index));
fclose(opt_parameters_purcell);

surf(wave_length_array/r,helix_rad_array/r,abs(purcell_ineff'));
view(0,90);
colorbar;
xlabel("\lambda/r")
ylabel("R/r")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_purcell_ineff_heatmap.png'])
...
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_purcell_ineff_log_heatmap.png'])

ineff_cap=400;
truncated_purcell_ineff=purcell_ineff.*double(purcell_ineff<ineff_cap);
truncated_purcell_ineff(truncated_purcell_ineff==0)=1;

surf(wave_length_array/r,helix_rad_array/r,abs(truncated_purcell_ineff'));
view(0,90);
colorbar;
xlabel("\lambda/r")
ylabel("R/r")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_truncated_',num2str(ineff_cap),'_purcell_ineff_heatmap.png'])
...
ylabel("R/r")
set(gca,"Colorscale","log")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_truncated_',num2str(ineff_cap),'_purcell_ineff_log_heatmap.png'])



%% 
min_energy_per_dist = min(abs(energy_per_dist(:)));
[min_energy_wl_index,min_energy_hrad_index]=find(abs(energy_per_dist)==min_energy_per_dist);
opt_wl_energy = wave_length_array(min_energy_wl_index)/r;
opt_hrad_energy = helix_rad_array(min_energy_hrad_index)/r;

opt_parameters_energy_per_dist=fopen([dir,'/scaled_optimal_wavelength_and_helixrad_energy_per_dist_motorspeed_',num2str(2*pi*freq),'.txt'],"w");
fprintf(opt_parameters_energy_per_dist,'The minimum energy cost per distance is %12.4g\n. The corresponding optimal wavelength (over r) is %12.4g\n The corresponding optimal helix radius (over r) is %12.4g\n',min_energy_per_dist,wave_length_array(min_energy_wl_index)/r,helix_rad_array(min_energy_hrad_index)/r);
fclose(opt_parameters_energy_per_dist);

surf(wave_length_array/r,helix_rad_array/r,abs(energy_per_dist'));
view(0,90);
colorbar;
xlabel("\lambda/r")
ylabel("R/r")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_energy_per_dist_heatmap.png'])
...
ylabel("R/r")
set(gca,"Colorscale","log")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_energy_per_dist_log_heatmap.png'])

energy_cap=80000;
truncated_energy_per_dist=energy_per_dist.*double(energy_per_dist<energy_cap);
truncated_energy_per_dist(truncated_energy_per_dist==0)=1;

surf(wave_length_array/r,helix_rad_array/r,abs(truncated_energy_per_dist'));
view(0,90);
colorbar;
xlabel("\lambda/r")
ylabel("R/r")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_truncated_',num2str(energy_cap),'_energy_per_dist_heatmap.png'])
...
ylabel("R/r")
set(gca,"Colorscale","log")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_truncated_',num2str(energy_cap),'_energy_per_dist_log_heatmap.png'])

end

%% 
%{
plot(wave_length_array/r,purcell_in_eff)
xlabel("\lambda/r")
ylabel("Purcell inefficiency")
saveas(gcf,[dir,'/motorspeed_',num2str(2*pi*freq),'_purcell_ineff_vs_wavelength.png'])
%}
