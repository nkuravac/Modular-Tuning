...
%%%
%Most recent edit 6/12: Updated to simulate Pseudomonas aeruginosa (PA).
...
bigdir = ['../../data/full_sweep_PA',char(datetime('now','Format','MM-dd-yyyy_HH-mm'))];
mkdir(bigdir)
...
arclen_array=linspace(5*r,15*r,30);
wave_length_array=linspace(r,8*r,9);
hrad_array=linspace(0.3*r,0.8*r,9);
...
arclen_array=sort([arclen_array,opt_arclen_purcell,L_Wu]);
...
writetable(full_sweep_table,[summarydir,'/full_sweep_table_PA.txt']);
