%hard-coded stuff based on my machine in this one as well. I don't think
%this script is necessary though, since this fixes stall torque which
%produced strange results. It makes more sense to fix max power, as in 
%energy_per_dist_optimize.
...
opt_wl_previous_index=find(abs(wave_length_array-3.3938)<0.1);
wave_length=wave_length_array(opt_wl_previous_index);

R=0.485;
base_freq=154;

n_loadlines=length(arclen_array);
n_tslines=length(stall_torque_vec);
...
bigdir = ['../../data/arclen_investigation_',char(datetime('now','Format','MM-dd-yyyy_HH-mm'))];
mkdir(bigdir)
...
for repeat=1:2
    ...
    if repeat==1
        ...
        freq=stall_torque_vec(j)/(ex_load_line_slopes(i)+stall_torque_vec(j)/all_x_ints(j));
        freq_array(i,j)=freq;
    end
...
bggrad=color_gradient([0 0 1],[0.2 1 2/3],n_tslines);
for i=1:n_loadlines
