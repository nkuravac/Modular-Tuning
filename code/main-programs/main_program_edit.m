...
%wave_length_array = linspace(7*R,10*R,10);
...
purcell_eff = zeros(length(wave_length_array),1);
%wave_length=0;
...
%for i_wl=1:splits
    ...
    for i_cb = 1:length(cell_a)
        r = cell_a(i_cb);
        height = height_a;
        dir = ['../../data/wlength_',num2str(wave_length),'_cell_rad_',num2str(r)];
        if(~exist(dir,'dir'))
            mkdir(dir)
        else
            rmdir(dir,'s')
...
%}
purcell_eff(i_wl)=avg_sumFf(3)*avg_U_model(3)/(avg_sumTf(3)*avg_Omega_model(3));
end
purcell_in_eff=1./purcell_eff
[min_ineff,min_ineff_index]=min(purcell_in_eff);
wave_length_array(min_ineff_index)
